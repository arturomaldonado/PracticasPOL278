...
class(PP2024$Ejecucion) #vemos de que clase es la variable ejecución de la bbdd
```
...
[^1]: *Más información del artículo al siguiente [link](https://pubmed.ncbi.nlm.nih.gov/8528684/)*
...
Aunque el programa se detuvo en 1989 después de que la primera clase de jardín de infancia del programa terminara el tercer grado, la recopilación de información (por ejemplo, el rendimiento en los exámenes de octavo grado, el promedio general de la escuela secundaria) continuó hasta el final de la asistencia de los participantes a la escuela secundaria. *Para esta sesión **no** vamos a examinar las hipótesis o resultados del trabajo de Mosteller (1995), pero sí vamos a utilizar parte de su base de datos para el análisis descriptivo* 😌

> **¿Por qué son importantes este tipo de estudios desde las ciencias sociales ?**
>
...
```{r}
data$race = factor(data$race, levels = c(1:6), labels = c("white","black","asian","hispanic","native american","others")) #Otorgamos formato; "factor" quiere decir que se otorga formato de variable categórica
...
La tabla anterior se llama **tabla de frecuencias**, y nos permite observar cuántas observaciones existen por cada variable.

En el caso anterior, la tabla nos indica que, de todas las personas estudiadas, 1383 se identificaron como *white*, 211 como *black*, 5 como *asian* y 1 como *others*. Asimismo, se observa que nadie se identificó como *native american* ni *hispanic*.

Asimismo, podemos explorar las frecuencias de manera gráfica con la ayuda del paquete `ggplot2` . Para ello, antes debemos de crear una tabla y crear un objeto, lo llamaremos *para_grafico*
...
Si no contamos con el paquete ggplot lo instalamos insertando el siguiente código en un chunk: `install.packages("ggplot2")`
...
ggplot(para_graficoRace, aes(x=race, y=Freq, fill=race)) + 
geom_bar(stat = "identity")+
  ggtitle("Distribución de autopercepción") +
...
Al igual que con la tabla anterior, en este caso la \*\*tabla de frecuencia\* nos indica que, de los encuestados, 481 pertenecen a una clase pequeña, 578 pertenecen a una clase regular y otros 541 pertenecen a una clase regular con ayuda. También lo podemos ver en gráfico con el paquete ggplot2
...
geom_bar(stat = "identity")+
  ...
  ggtitle("Distribución de tipo de clase") +
...
geom_bar(stat = "identity")+
  ...
  ggtitle("Distribución de graduados") +
  xlab("Tipo de clase") + ylab("Frecuencia")+
...
```{r}
para_graficoMath = data %>%
  group_by(g4math) %>%
   summarize(Freq=n())
```

Como nuestra variable (g4math) es numérica, esta vez pediremos un **histograma**. Es similar a un gráfico de barras, solo que esta vez todas las barras se encuentran juntas y no separadas.

```{r}
library(ggplot2) #no olvidar llamar al paquete
ggplot(para_graficoMath, aes(x=g4math, y=Freq)) + 
...
geom_histogram(fill ="blue", stat = "identity") +
 ...
 ggtitle("Notas de matemática") +
  xlab("Puntaje") + ylab("Frecuencia") +
  theme(panel.background=element_rect(fill = "lightgray", colour = "black"))
```
El histograma muestra una **distribución asimétrica sesgada con sesgo negativo o a la izquierda**. Esto quiere decir que existe una concentración de datos hacia la derecha.

También podemos calcular un **gráfico de cajas o "boxplot** a partir de la misma información numérica.

```{r}
ggplot(para_graficoMath, aes(y=Freq)) + 
geom_boxplot(star = "identity")
```
También se puede personalizar

```{r}
ggplot(para_graficoMath, aes(x = " ", y=Freq)) + 
geom_boxplot(star = "identity", fill = "yellow", outlier.colour = "orange") +
  
  stat_boxplot(geom = "errorbar", width = 0.15)
```
Este gráfico permite observar la mediana (línea gruesa dentro de la caja), así como valores atípicos. En este caso, se pueden observar 3 valores atípicos (círculos naranjas).

Los extremos de la caja indican el percentil 25 y el percentil 75. Quiere decir que entre el valor ubicado entre el 25% y 75% de valores se encuentran dentro de la caja. Los "Bigotes" indican los valores debajo del percentil 25 y el percentil 75.
