...
str(PP2023) # Visualice la estructura de la bbdd
names(PP2023) #Visualice los nombres de las variables de la bbdd
```
...
data %>% summarise(media = mean(g4math)) #este comando nos permitirá calcular la media
```
...
```{r}
library(DescTools)
data %>% summarise(mediana = median(g4math)) #este comando nos permitirá calcular la mediana
```
