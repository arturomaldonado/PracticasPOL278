...
COLOCAR COMO PIE DE PÁGINA <https://www.data-to-viz.com/caveat/pie.html>
...
A primera vista, la tabla nos indica que la mayoría de los encuestados (818) opina que hay mucha desigualdad económica en el país. Pero, ¿cuánto sería dicho resultado en porcentaje?
...
Para dar un acercamiento a la percepción de la desigualdad en cuanto a la calidad de vida en el Perú se ha considerado cuatro variables:
...
5.  Restar el valor mínimo a la suma de todos los valores.
...
#ejemplo usando dplyr: data = data %>%  mutate(across(7:10, as.numeric))
```
...
```{r}
data = data %>%
 mutate(indicador = ((suma-4)/(12-4))*100) #Como queremos que el indicador vaya del 0 al 100, lo multiplicamos por 100
```
