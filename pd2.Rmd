...
Graficar información ayuda a contar historias y, sobre todo, *dar un sentido* a los cientos, miles, o incluso millones de filas de datos que con las que podríamos eventualmente trabajar, facilitando la comprensión de la información.
...
INTRODUCIT LA DATA, QUE ESTÉ SIMILAR AL FORMATO DE LA PD1.
...
## **4.1 ¿Cuál es el porcentaje de percepción de desigualdad económica en el Perú?**

Usaremos la pregunta P04: ¿Qué tan desigual cree que es el Perú económicamente?
...
Pasos para analizar una variable ordinal 
1. Identificar el tipo de variable (Es decir, como lo esta identificando RStudio)
2. Convertimos la variable al tipo que necesitamos
3. Elaboramos un gráfico que nos permita ver preliminarmente los datos de la variable. 
4. Elaboramos un gráfico que vaya acorde a la variable ordinal (gráfico de barras)
...
#Primero analizaremos el tipo de dato que analizaremos
class(data$p04)
```
...
```{r}
data$p04 = factor(data$p04, levels = c(1:4), labels = c("Mucho","Algo","Poco","Nada"),ordered = TRUE) #Le otorogamos las etiquetas de Mucho, Algo, Poco y Nada
```

<<<<<<< HEAD
Revisemos que el cambio se haya realizado correctamente usando el comando summarise del paquete `dplyr`
=======
Revisemos que el cambio se haya realizado correctamente. Para hacerlo tenemos dos formas, 1. comando table (que revisamos anteriormente), y 2. commando summarize/summarise de dplyr (Para esta última forma recordemos que se debe convocar a la libreria dplyr)
>>>>>>> b82245c87053647ceab7adc650946d75d3e4a833
...
A primera vista, la tabla nos indica que la mayoría de los encuestados (818) opina que hay mucha desigualdad económica en el país. Pero, ¿cuánto sería eso en porcentaje?
...
HACER INTRO SOBRE CALIDAD DE VIDA Y LA ELECCIÓN DE LAS VARIABLES DE LA ENCUESTA
...
EXPLICACIÓN DEL BOXPLOT. Antuané
