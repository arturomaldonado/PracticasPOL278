...
```{r}
data = data %>%
 mutate(ESTRATO = factor(ESTRATO, labels = c("A","B","C","D","E"), ordered = TRUE))
```
...
```{r}
data = data %>%
 mutate(REGIONNATU = factor(REGIONNATU, levels = 1:3, labels = c("Costa","Sierra","Selva")))
```
...
## 5.¿Cuál es la proporción de personas con mucha confianza en la PNP ?

Para responder a la pregunta calcularemos el *intervalo de confianza para una proporción*. Primero se tiene que definir qué es tener mucha confianza en la PNP, para ello crearemos una variable *dicotómica* a partir de `confianza_pnp`

```{r}
data = data %>%
  mutate(
    confianza_dico = case_when(
      confianza_pnp==1|confianza_pnp==2 ~ 1,
      confianza_pnp==3|confianza_pnp==4 ~ 2))
...
```{r}
data = data %>%
  mutate(confianza_dico = factor(confianza_dico, levels = 1:2, labels = c("Poca","Mucha"), ordered = TRUE))
table(data$confianza_dico)
```

Caso de éxito 💥

```{r}
casos_exi = 6365 #almacenamos en "casos_exi" la frecuencia de nuestro caso de éxito (Mucha confianza)
total = 6365 + 8625 #almacenamos en "total" el total de casos
```

```{r}
ic_prop = prop.test(casos_exi,total)
...
```{r}
ggplot(para_graficoP, aes(x=P40_1,y=Porcentaje,fill=P40_1))+
  geom_bar(stat="identity")+
